...
def add_eco_ci_steps(yaml_data):
    ...
    for job_name, job in modified_yaml_data.get("jobs", {}).items():
        ...
        # Inject energy measurement start step
        steps.insert(0, {
        ...
        for step in steps:
            ...
            # Skip tracking for artifact/upload/display/energy steps
            step_name = step.get("name", "").lower()
            if (
                "upload" in step_name or
                "display" in step_name or
                "energy" in step_name or
                "save" in step_name
            ):
                continue
